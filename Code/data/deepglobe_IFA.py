...
class DatasetDeepglobeIFA(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        query_img_test, query_mask_test, support_imgs, support_masks, \
            query_sam, support_sams = self.load_frame(query_name, support_names)
        
        # query_sam 40 x H x W, support_sams = shot x (40 x H x W) 
        ...
        query_mask = F.interpolate(query_mask.unsqueeze(0).unsqueeze(0).float(), query_img.size()[-2:], mode='nearest').squeeze() # 1 x H x W
        query_mask = query_mask.long() # h x w
        ...
        support_masks = torch.stack(support_masks_tmp) # shot x h x w
    ...
    def load_frame(self, query_name, support_names):
        ...
        query_id = query_name.split('/')[-1].split('.')[0] # 919051_sat_22
        q_msk_id = query_name.split('/')[-1][:-11] + '_mask_' + query_name.split('/')[-1][-6:-4] # 919051_mask_22
        ann_path = os.path.join(self.base_path, query_name.split('/')[-4], 'test', 'groundtruth') # ./dataset/Deepglobe/04_train_cat/1/test/groundtruth
        query_name = os.path.join(ann_path, q_msk_id) + '.png' #./dataset/Deepglobe/04_train_cat/1/test/groundtruth/444902_mask_14.png
